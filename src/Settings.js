...
export default function Settings() {
  ...
  const [privacySettings, setPrivacySettings] = useState({ showOnlineStatus:true, showLastSeen:true, showProfilePhoto:true });
  ...
  const showToast = (message, type='info') => { setToastMessage(message); setToastType(type); setToastVisible(true); setTimeout(() => setToastVisible(false), 3000); };

  const copyCPCode = () => { if (!currentUser?.cpCode) return; navigator.clipboard?.writeText(currentUser.cpCode); setCopied(true); setTimeout(() => setCopied(false), 2000); showToast('CP code copied!', 'success'); };

  const handleChangeCpCode = async e => {
    e.preventDefault(); setCpCodeError(''); setCpCodeSuccess('');
    if (!/^\d{10}$/.test(newCpDigits)) { setCpCodeError('Enter exactly 10 digits (0-9)'); return; }
    const fullCode = 'CP-' + newCpDigits;
    try { await changeUserCpCode(currentUser.uid, fullCode); setCpCodeSuccess('CP code updated!'); showToast('CP code updated successfully!', 'success'); setNewCpDigits(''); setTimeout(() => window.location.reload(), 2000); }
    catch(err) { setCpCodeError(err.message); showToast(err.message, 'error'); }
  };

  const handleResetPassword = async () => {
    try { await sendPasswordResetEmail(auth, currentUser.email); setPasswordResetSent(true); showToast('Password reset email sent!', 'success'); }
    catch(err) { setCpCodeError(err.message); showToast(err.message, 'error'); }
  };

  const handleDeleteAccount = async () => {
    try { const user = auth.currentUser; if (user) { await user.delete(); showToast('Account deleted', 'info'); window.location.reload(); } }
    catch(err) { setCpCodeError('Could not delete account: ' + err.message); showToast('Delete failed: ' + err.message, 'error'); }
  };

  const handleSaveStatus = async () => {
    try { await updateDoc(doc(db, 'users', currentUser.uid), { status }); setStatusSaved(true); showToast('Status updated!', 'success'); setTimeout(() => setStatusSaved(false), 2000); }
    catch(err) { showToast(err.message, 'error'); }
  };

  const handleSaveDisplayName = async () => {
    if (!displayName.trim()) { showToast('Display name cannot be empty', 'error'); return; }
    try { await updateProfile(auth.currentUser, { displayName }); await updateDoc(doc(db, 'users', currentUser.uid), { displayName }); setDisplayNameSaved(true); showToast('Display name updated!', 'success'); setTimeout(() => setDisplayNameSaved(false), 2000); }
    catch(err) { showToast(err.message, 'error'); }
  };

  const handleExportData = async () => {
    try { const userDoc = await getDoc(doc(db, 'users', currentUser.uid)); const json = JSON.stringify(userDoc.data(), null, 2); const blob = new Blob([json], { type:'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `connect-point-data-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(url); showToast('Data exported!', 'success'); }
    catch(err) { showToast('Export failed: ' + err.message, 'error'); }
  };

  const lastChanged = currentUser?.cpCodeLastChanged ? new Date(currentUser.cpCodeLastChanged.seconds * 1000).toLocaleDateString() : null;
  ...
  const profileCard = React.createElement('div', { className:'glass', style:{ padding:'1.5rem', marginBottom:'16px', borderRadius:'20px' } },
    React.createElement('div', { style:{ display:'flex', flexDirection:'column', alignItems:'center', marginBottom:'1.5rem' } },
      React.createElement('div', { style:{ position:'relative', width:'100px', height:'100px', borderRadius:'50%', background:'var(--accent)', display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden' } },
        currentUser.photoURL ? React.createElement('img', { src:currentUser.photoURL, alt:'avatar', style:{ width:'100%', height:'100%', objectFit:'cover' } }) : React.createElement('i', { className:'ph ph-user-circle', style:{ fontSize:'4rem', color:'white' } }),
        React.createElement('button', { style:{ position:'absolute', bottom:'4px', right:'4px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'50%', width:'32px', height:'32px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', backdropFilter:'blur(10px)' }, onClick:() => showToast('Profile picture upload coming soon!', 'info') }, React.createElement('i', { className:'ph ph-camera' }))
      ),
    ...
    React.createElement('div', { style:{ marginTop:'1rem' } },
      ...
      React.createElement('div', { style:{ display:'flex', gap:'8px', marginTop:'4px' } },
        React.createElement('input', { className:'input-field', type:'text', value:displayName, onChange:e => setDisplayName(e.target.value), placeholder:'Your display name', style:{ flex:1 } }),
    ...
    React.createElement('div', { style:{ marginTop:'1rem' } },
      ...
      React.createElement('div', { style:{ display:'flex', gap:'8px', marginTop:'4px' } },
        React.createElement('input', { className:'input-field', type:'text', value:status, onChange:e => setStatus(e.target.value), placeholder:'Add a short bio...', maxLength:100, style:{ flex:1 } }),
        React.createElement('button', { className:'btn btn-primary', onClick:handleSaveStatus, style:{ padding:'0 20px' } }, statusSaved ? 'Saved!' : 'Save')
      ),
    ...
    React.createElement('div', { style:{ marginTop:'1.5rem' } },
      ...
      React.createElement('div', { style:{ display:'flex', alignItems:'center', gap:'8px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'12px', padding:'12px', margin:'8px 0' } },
        ...
        React.createElement('button', { className:'btn-icon', onClick:copyCPCode, title:'Copy' }, React.createElement('i', { className:`ph ${copied ? 'ph-check' : 'ph-copy'}` }))
      ),
      ...
      canChange && React.createElement('form', { onSubmit:handleChangeCpCode, style:{ marginTop:'12px' } },
        React.createElement('div', { className:'input-group' },
          ...
          React.createElement('div', { style:{ display:'flex', alignItems:'center', gap:'8px' } },
            ...
            React.createElement('input', { className:'input-field', type:'text', inputMode:'numeric', pattern:'\\d{10}', maxLength:10, placeholder:'1234567890', value:newCpDigits, onChange:e => setNewCpDigits(e.target.value.replace(/\D/g, '').slice(0,10)), required:true, style:{ flex:1 } })
          )
        ),
        cpCodeError && React.createElement('div', { className:'error-msg' }, cpCodeError),
  ...
  const securityCard = React.createElement('div', { className:'glass', style:{ padding:'1.5rem', marginBottom:'16px', borderRadius:'20px' } },
    ...
    showDeleteConfirm
      ? React.createElement('div', { className:'fade-in', style:{ display:'flex', gap:'8px' } },
          React.createElement('button', { className:'btn', style:{ background:'var(--danger)', color:'white', flex:1 }, onClick:handleDeleteAccount }, React.createElement('i', { className:'ph ph-trash' }), ' Confirm Delete'),
  ...
  const preferencesCard = React.createElement('div', { className:'glass', style:{ padding:'1.5rem', marginBottom:'16px', borderRadius:'20px' } },
    ...
    React.createElement('div', { style:{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid var(--border)' } },
      ...
      React.createElement('button', { className:'btn', onClick:toggleTheme, style:{ padding:'4px 12px', fontSize:'0.85rem' } }, React.createElement('i', { className:'ph ph-sun' }), ' / ', React.createElement('i', { className:'ph ph-moon' }), ' Toggle')
    ),
    React.createElement('div', { style:{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid var(--border)' } },
    ...
    React.createElement('div', { style:{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 0' } },
      ...
      React.createElement('select', { value:'en', style:{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'8px', padding:'4px 8px', color:'var(--text-primary)' } },
        React.createElement('option', { value:'en' }, 'English'), React.createElement('option', { value:'es' }, 'Spanish'), React.createElement('option', { value:'fr' }, 'French'), React.createElement('option', { value:'de' }, 'German')
      )
    )
  );
