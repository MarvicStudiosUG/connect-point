...
export function ToastProvider({ children }) {
  ...
  return React.createElement(
    ...
    { value: { addToast } },
    children,
    React.createElement('div', { style: { position: 'fixed', bottom: '90px', left: '50%', transform: 'translateX(-50%)', zIndex: 10000, display: 'flex', flexDirection: 'column-reverse', gap: '8px' } },
...
function ToastItem({ toast, onClose }) {
  const bg = toast.type === 'success' ? 'rgba(34,197,94,0.95)' : toast.type === 'error' ? 'rgba(239,68,68,0.95)' : 'rgba(127,90,240,0.95)';
